...
export class SideBarComponent implements OnInit {
  ...
  async fillColors(){
    let colors: string[] = Object.assign([],this.colorList)
    // First remove used colors
    for(const ind in this.apiService.allTasks){
      if(this.apiService.allTasks[ind].color !== undefined){
        colors = <string[]>this.removeItemOnce(colors,<string>this.apiService.allTasks[ind].color)
      }
    }
    ...
    for(const ind in this.apiService.allTasks){
      ...
      if(this.apiService.allTasks[ind].color === undefined){
        let id: string =  <string>this.apiService.allTasks[ind].id
        this.apiService.updateTask({ color: `bg-${color}-500`},id)
      }
    }
  }

  removeItemOnce(arr: string[], value: string) {
